...
$sql = "SELECT harvest.id,crop.crop_type,qty_kg,price,picture_1,date,status FROM harvest, crop WHERE harvest.crop_type_id=crop.id AND harvest.farmer_id='$id' AND harvest.expiry_timestamp > NOW() AND (harvest.status IS NULL OR harvest.status='Purchased') ORDER BY farmer_id,date desc";
